...
export default function ProjectDetailPage() {
  ...
  return (
    <div className="space-y-6">
      {/* ✅ FIX 1 — Header: column on mobile, row on sm+ */}
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="min-w-0">
          {/* ✅ FIX 2 — Title row: allow wrapping so badges don't overflow */}
          ...
          <p className="text-slate-400 text-sm sm:ml-8">{project.description}</p>
        </div>

        {/* ✅ FIX 1 cont. — removed flex-shrink-0, added flex-wrap so buttons never hide */}
      ...
      {tab === 'tasks' && (
        <div className="space-y-4">
          {/* ✅ FIX 3 — Filters row: stack on mobile, row on sm+ */}
          <div className="flex flex-col sm:flex-row sm:items-center gap-3">
            <TaskFilters onFilter={loadTasks} />
            {/* ✅ FIX 3 cont. — sm:ml-auto so it stays right on larger screens */}
          ...
          {/* ✅ FIX 4 — Bulk actions: stack on mobile, row on sm+ */}
          {selected.length > 0 && canEdit && (
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 p-3 glass rounded-xl border border-brand-500/20 animate-fade-in">
              <span className="text-sm text-slate-400">{selected.length} selected</span>
              {/* ✅ FIX 4 cont. — flex-wrap so buttons wrap instead of overflow */}
        ...
        <div className="space-y-3">
          {project.members?.map((m) => {
            ...
            return (
              <div key={m.user?._id} className="card flex items-center gap-4">
                <Avatar user={m.user} size="md" />
                {/* ✅ FIX 5 — overflow-hidden prevents long emails from breaking layout */}
                <div className="flex-1 min-w-0 overflow-hidden">
                  <p className="font-medium text-slate-200 truncate">{m.user?.name}</p>
                  <p className="text-xs text-slate-500 truncate">{m.user?.email}</p>
                </div>
                {/* ✅ FIX 5 cont. — flex-shrink-0 prevents badge from being squished */}
                <Badge color={role?.color} bg={role?.bg} className="flex-shrink-0">{role?.label}</Badge>
                {/* ✅ Only admin can remove members */}
                {isAdmin && m.user?._id !== user?._id && (
                  <button className="text-slate-600 hover:text-red-400 transition-colors p-1 rounded hover:bg-red-400/10 flex-shrink-0">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                    </svg>
                  </button>
